...
public class GenericItemRow implements RowDataToolTip {
    private static final Logger LOGGER = LoggerFactory.getLogger((Class) GenericItemRow.class);
    ...
    private GenericItemDataObject dataObject; //this is the object that is tied to the row. TABLE COLUMN TYPES NEED TO MATCH THE DATA TYPE OF THE CLASS PROPERTIES

    public GenericItemRow(GenericItemDataObject dataObject) {
...
//        colPickLocation = new FBTableColumn(colCount++, "Pick Location", PointOfSaleLocation.class, 100, "", FBTableColumnEditable.ON_CLICK, false, false);
    ...
    public Object[] getRow() {
        final Object[] values = new Object[GenericItemRow.colCount]; //array to hold the column values

        //ITERATE THE DATA OBJECT PROPERTIES AND ASSIGN THEM TO THE MATCHING COLUMN INDEX IN THE ARRAY
        values[GenericItemRow.colDataObjectId.getColIndex()] = dataObject.getId();
        values[GenericItemRow.colDataObjectName.getColIndex()] = dataObject.getName();
    ...
    public static FBTableColumnSettings getSettings(){
        ...
        //ADD ALL COLUMNS
        settings.addColumn(GenericItemRow.colDataObjectId);
        settings.addColumn(GenericItemRow.colDataObjectName);
    ...
    public String getToolTip(final int columnIndex) {
        ...
        if (columnIndex == GenericItemRow.colDataObjectName.getColIndex()) {
